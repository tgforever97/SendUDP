...
class Message {
 ...
 public:
  ...
  static void sendMsg(const Message& msg, int sock, const struct sockaddr * remAddr, socklen_t remAddrLen) {
    static uint8_t buf[MSG_SEND_BUF_SIZE]{0};
    size_t len = msg.getLength();
    msg.getBinary(buf, MSG_SEND_BUF_SIZE);
    sendto(sock, buf, len, 0, remAddr, remAddrLen);
    memset(buf, 0, len);
  }
...
class RttTestMsg : public Message {
 public:
  ...
  RttTestMsg(int packetSize, int testId, int mid) : Message(msgType, testId, mid), payloadLen(packetSize - 15) {}
